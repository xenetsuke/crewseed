    ...
    <div className="card p-6 hover:shadow-lg transition-all duration-200">
          ...
          <div className="flex items-start justify-between gap-2 mb-1">
            ...
            {assignment?.status && (
              <span className={`badge ${statusInfo?.className} flex-shrink-0`}>
                ...
                {t(`assignmentStatus.${assignment.status}`, assignment.status)}
              </span>
            )}
          </div>
      ...
      <div className="space-y-3 mb-4">
        ...
        <div className="flex items-center gap-2 text-sm">
          ...
          <span className="text-muted-foreground">
            {t('assignments.deadline', 'Deadline')}: {assignment?.deadline}
          </span>
        </div>
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          ...
          {assignment?.skills?.length > 3 && (
            <span className="px-2 py-1 text-xs font-medium bg-muted text-muted-foreground rounded-full">
              +{assignment?.skills?.length - 3} {t('common.more', 'more')}
            </span>
          )}
        </div>
      )}
          ...
          <Button
            ...
            onClick={() =>
              navigate('/assignment-details', {
                state: { assignment, applyMode: true },
              })
            }
          >
            {t('common.applyNow')}
