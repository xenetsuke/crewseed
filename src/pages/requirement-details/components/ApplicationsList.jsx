...
const ApplicationsList = ({
  ...
  useEffect(() => {
    const loadProfiles = async () => {
      try {
        ...
        const responses = await Promise.all(
          uniqueIds.map((id) =>
            getProfile(id).then((profile) => ({ id, profile }))
          )
        );
        const map = {};
        responses.forEach((r) => { map[r.id] = r.profile; });
        setProfiles(map);
      } catch (err) {
    ...
    if (applications.length > 0) loadProfiles();
  ...
  const STATUS_UI = {
    APPLIED: { label: "Applied", badge: "bg-amber-500/10 text-amber-600 border-amber-500/20", icon: "Clock" },
    UNDER_REVIEW: { label: "Reviewing", badge: "bg-blue-500/10 text-blue-600 border-blue-500/20", icon: "Eye" },
    INTERVIEWING: { label: "Interview", badge: "bg-purple-500/10 text-purple-600 border-purple-500/20", icon: "Calendar" },
    SELECTED: { label: "Selected", badge: "bg-emerald-500/10 text-emerald-600 border-emerald-500/20", icon: "CheckCircle" },
    REJECTED: { label: "Rejected", badge: "bg-destructive/10 text-destructive border-destructive/20", icon: "XCircle" },
    JOINED: { label: "Joined", badge: "bg-emerald-600 text-white border-emerald-700", icon: "UserCheck" },
  };

  const getStatusUI = (status) =>
    STATUS_UI[status] || { label: status, badge: "bg-muted text-muted-foreground border-border", icon: "HelpCircle" };
      ...
      <div className="divide-y divide-border">
        {applications.length === 0 ? (
          <div className="text-center py-16 bg-muted/10">
            <Icon name="Inbox" size={32} className="text-muted-foreground/40 mx-auto mb-4" />
          ...
          applications.map((app) => {
            ...
            return (
              <div key={app.applicantId} className="group p-4 sm:p-5 hover:bg-muted/30 transition-all">
                <div className="flex flex-col lg:flex-row lg:items-start gap-4 sm:gap-6">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-3 text-muted-foreground mb-2">
                         <span className="text-[11px] sm:text-xs flex items-center gap-1 font-medium">
                          <Icon name="MapPin" size={12} /> {profile.currentCity || "N/A"}
                        </span>
                        <span className="text-[11px] sm:text-xs flex items-center gap-1 font-medium border-l border-border pl-3">
                          <Icon name="Award" size={12} /> {profile.totalExperience || 0}y Exp
                        </span>
                      </div>

                      <div className="flex flex-wrap gap-1.5">
                        {profile.skills?.slice(0, 2).map((skill, i) => (
                          <span key={i} className="px-1.5 py-0.5 text-[9px] font-bold rounded bg-muted text-muted-foreground border border-border uppercase">
                            {skill}
                          </span>
                        ))}
                      </div>
                  ...
                  <div className="flex flex-col gap-3 lg:w-48 lg:pl-6 lg:border-l border-border">
                    ...
                    {/* Status Badge: Prominent on all screens */}
                    <div className="flex items-center lg:justify-center">
                       <span className={`w-full text-center inline-flex items-center justify-center gap-1.5 px-3 py-1.5 rounded-lg text-[10px] font-black border uppercase tracking-widest ${statusUI.badge}`}>
                ...
                {(isExpanded || app.applicationStatus === "APPLIED") && (
                  <div className="mt-4 pt-4 border-t border-dashed border-border overflow-x-auto">
                    <div className="flex flex-row items-center gap-2 pb-1">
                      <Button size="sm" variant="outline" className="border-blue-200 text-blue-700 bg-blue-50/50 hover:bg-blue-100 font-bold text-[10px] shrink-0" iconName="CheckCircle" onClick={() => onApprove(app)}>
                        Shortlist
                      </Button>
                      <Button size="sm" variant="outline" className="border-emerald-200 text-emerald-700 bg-emerald-50/50 hover:bg-emerald-100 font-bold text-[10px] shrink-0" iconName="UserCheck" onClick={() => onSelect(app)}>
                        Select
                      </Button>
