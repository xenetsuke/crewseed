...
=======
            <h1 className="text-2xl font-bold text-slate-900">{requirement?.jobTitle || "Untitled Job"}</h1>
            <span className={`px-3 py-1 rounded-full text-xs font-bold uppercase ${getStatusColor(rawStatus)}`}>{normalizedStatus}</span>
          </div>

          <div className="flex flex-wrap items-center gap-4 text-sm text-slate-500">
            <div className="flex items-center gap-2"><Icon name="MapPin" size={16} /><span>{requirement?.fullWorkAddress || "Remote"}</span></div>
...
=======
          <Button variant="outline" size="sm" iconName="RefreshCcw" iconPosition="left" onClick={onEdit}>Edit/Renew</Button>
>>>>>>> Stashed changes
...
=======
          <div className="flex items-center gap-2 mb-1"><Icon name="IndianRupee" size={18} className="text-blue-600" /><span className="text-xs font-semibold text-blue-600 uppercase">Salary</span></div>
          <p className="text-lg font-bold text-slate-800">{compensationText}</p>
        </div>
        <div className="p-4 rounded-xl bg-purple-50 border border-purple-100">
          <div className="flex items-center gap-2 mb-1"><Icon name="Users" size={18} className="text-purple-600" /><span className="text-xs font-semibold text-purple-600 uppercase">Openings</span></div>
          <p className="text-lg font-bold text-slate-800">{requirement?.numberOfWorkers || 0} Positions</p>
        </div>
        <div className="p-4 rounded-xl bg-orange-50 border border-orange-100">
          <div className="flex items-center gap-2 mb-1"><Icon name="Briefcase" size={18} className="text-orange-600" /><span className="text-xs font-semibold text-orange-600 uppercase">Experience</span></div>
          <p className="text-lg font-bold text-slate-800">{formatEnum(requirement?.experienceLevel) || "Freshers"}</p>
        </div>
        <div className="p-4 rounded-xl bg-emerald-50 border border-emerald-100">
          <div className="flex items-center gap-2 mb-1"><Icon name="Timer" size={18} className="text-emerald-600" /><span className="text-xs font-semibold text-emerald-600 uppercase">Shift</span></div>
          <p className="text-lg font-bold text-slate-800">{formatEnum(requirement?.shiftType) || "Day Shift"}</p>
>>>>>>> Stashed changes
        </div>
      </div>
    </div>
